...
This package lives at `owasp_skill_checker/` inside the **sdaitoolkit** repo.
...
```bash
cd owasp_skill_checker   # from the sdaitoolkit repo root
...
| `AST01-EXT-SCRIPT` | Invokes an external script or Python file: `python3 helper.py`, `bash install.sh`, `./setup.py`, `exec(open("x.py"))`, `subprocess.run(["python", "x.py"])`, `source hooks.sh`, `run helper.py` |
...
From `owasp_skill_checker/` (inside the sdaitoolkit repo):
